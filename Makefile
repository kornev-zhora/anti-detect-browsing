...
kameleo-start: ## Start Kameleo container
	...
	docker-compose -f $(KAMELEO_COMPOSE) up -d
...
kameleo-stop: ## Stop Kameleo container
	...
	docker-compose -f $(KAMELEO_COMPOSE) down
...
kameleo-logs: ## Show Kameleo logs (follow)
	docker-compose -f $(KAMELEO_COMPOSE) logs -f
...
multilogin-build: ## Build Multilogin Docker image
	...
	cd multilogin-unofficial && docker-compose -f docker-compose.multilogin.yml build
	@echo "$(GREEN)✅ Build complete$(NC)"

multilogin-start: ## Start Multilogin container
	@echo "$(GREEN)Starting Multilogin...$(NC)"
	cd multilogin-unofficial && docker-compose -f docker-compose.multilogin.yml up -d
...
multilogin-stop: ## Stop Multilogin container
	...
	cd multilogin-unofficial && docker-compose -f docker-compose.multilogin.yml down
...
monitoring-stop: ## Stop monitoring stack
	...
	docker-compose down prometheus grafana cadvisor
...
clean-force: ## Force cleanup (no confirmation)
	...
	-docker-compose -f $(KAMELEO_COMPOSE) down -v 2>/dev/null
	-cd multilogin-unofficial && docker-compose -f docker-compose.multilogin.yml down -v 2>/dev/null
...
validate: ## Validate configuration files
	...
	@docker-compose config > /dev/null && echo "$(GREEN)✅ docker-compose.yml valid$(NC)" || echo "$(RED)❌ docker-compose.yml invalid$(NC)"
	@docker-compose -f $(KAMELEO_COMPOSE) config > /dev/null && echo "$(GREEN)✅ kameleo compose valid$(NC)" || echo "$(RED)❌ kameleo compose invalid$(NC)"
	@test -f .env && echo "$(GREEN)✅ .env exists$(NC)" || echo "$(RED)❌ .env missing$(NC)"
	@$(PYTHON) -m py_compile scripts/*.py && echo "$(GREEN)✅ Python scripts valid$(NC)" || echo "$(RED)❌ Python syntax errors$(NC)"
